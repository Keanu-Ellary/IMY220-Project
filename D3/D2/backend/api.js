...
async function signUp(name, surname, email, password, username, proPic, bio) {
	try {
		await client.connect();
		console.log("Connected to MongoDB");
...
async function playlistByUser(userId) 
{
	...
	try
	{
		...
		await client.connect(); // Connect once at the start
		console.log("Connected to MongoDB");
	...
	{
		await client.close(); // Close the connection after processing all playlists
	}
}
...
async function deleteUser(userId) 
{
	try
	{
		...
		console.log(user.username + "'s following is deleted");

		await comments.deleteMany({userID : usersID});
		console.log(user.username + "'s comments are deleted")
...
async function editSong(songId, title, artists, image, streams) 
{
	try 
	{
		await client.connect();
		...
		const exist = await collection.findOne({_id: new ObjectId(songId)});
		...
		if(artists && Array.isArray(artists)) fieldsToUpdate.artists = artists;
		if(image) fieldsToUpdate.image = image;
		if(streams) fieldsToUpdate.streams = parseInt(streams, 10);
...
async function addComment(userID, content, playlistID) 
{
	try 
	{
		const user = await userById(userID);
		...
		const result = await collection.insertOne(commentNew);
...
app.patch("/editProfile", async (req, res) => {
	const {userId, name, surname, username, proPic, bio} = req.body;
	...
	if(!name && !surname && !username && !proPic && !bio)
	{
	...
	try 
	{
		...
		const exist = await collection.findOne({_id : new ObjectId(userId)});
		...
		if(surname) edits.surname = surname;
		if(username) edits.username = username;
		if(proPic) edits.proPic = proPic;
		if(bio) edits.bio = bio;
		...
		{
			return res.status(304).send({message : "No changes were processed"});
		}

		res.status(200).send({message : "Update successful", edits});
	} 
	catch (error) 
	{
...
app.post('/like', async (req, res) => {
	...
	if(!commentID)
	{
		throw new Error("Comment ID is needed");
	}
	...
	try 
	{
		...
		const exist = await collection.findOne({_id : new ObjectId(commentID)});
		...
		return res.status(201).send({message : "Liked Comment", result});
	} 
	catch (error) 
	{
		console.error(error);
		return res.status(500).send({message : "Error in liking", error : error.message});
	}
});
...
app.post('/dislike', async (req, res) => {
	...
	try 
	{
		...
		return res.status(201).send({message : "Disliked Comment", result});
	} 
	catch (error) 
	{
		console.error(error);
		return res.status(500).send({message : "Error in disliking", error : error.message});
	}
});

app.post('/addComment', async (req, res) => {
	const {userID, content, playlistID} = req.body;

	try 
	{
		const added = await addComment(userID, content, playlistID);

		res.status(201).send({success : true, commentID : added});
	} 
	catch (error) 
	{
...
app.patch('/editSong', async(req, res) => {
	const {songId, title, artists, image, streams} = req.body;

	if (!songId) 
		{
        return res.status(500).send({ error: "Missing Song ID" });
    }
    if 
	(!title && !artists && !image && !streams) 
	{
	...
	try 
	{
		const returned = await editSong(songId, title, artists, image, streams);
		res.status(200).send({message : "Successfully updated song", returned});
	} 
	catch (error) 
	{
		res.status(500).send({message : "Error in updating", error : error.message});
	}
});
...
app.post("/signup", async (req, res) => {
	const { name, surname, email, password, username, proPic, bio } = req.body;
	...
	if(!proPic)
	{
		return res.status(400).send({message : "Profile URL is needed"});
	}

	if(!bio)
	{
		return res.status(400).send({message : "Bio is needed"});
	}
	...
	try {
		const result = await signUp(name, surname, email, password, username, proPic, bio);
